var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="player.points.lt('1e1000')"  class="overlayThing">{{tmp.p.youHaveDescription ? tmp.p.youHaveDescription:"You have"}} </span>
		<h2  class="overlayThing" id="points">{{format(player.points)}}</h2>
		<span v-if="player.points.lt('1e1e6')"  class="overlayThing"> {{tmp.e.pointsName}}</span>
		<br>
    ...
    'options-tab': {
        ...
        <table>
			...
			<tr>		
				<td><button class="opt" onclick="switchPause()">Pause: {{ player.p.paused ? "PAUSED" : "RUNNING"}}</button></td>
			</tr>
        </table>`
    },
